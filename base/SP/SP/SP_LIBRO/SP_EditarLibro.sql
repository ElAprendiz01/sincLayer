	...
	BEGIN
		...
		IF @Año_Publicacion IS NOT NULL AND (@Año_Publicacion < 1300 OR @Año_Publicacion > YEAR(GETDATE()))
		BEGIN
			SET @O_Numero = -1;
	...
	DECLARE @OutNumero INT;
DECLARE @OutMsg VARCHAR(255);

EXEC Editar_Tbl_Libros 
    @Id_Libro = 5,
    @Titulo = 'SQL Server',
    ...
    @Stock = 10,
    @Id_Modificador = 2,
    @O_Numero = @OutNumero OUTPUT,
    @O_Msg = @OutMsg OUTPUT;

SELECT @OutNumero AS CodigoRespuesta, @OutMsg AS MensajeRespuesta;
